        ...
        try {
            ...
            for (File propFile : getTargetPropertiesFiles()) {
               ...
               // String outputBarFile = new File(propFile.getParent(), propFile.getName().replaceAll("properties$", "bar")).getAbsolutePath();
                String propFileNameWithOutExt = FilenameUtils.removeExtension(propFile.getName());
                String outputBarFile = new File(barName.getParent(), barName.getName().replaceAll(".bar", "-" + versionString + "_" + propFileNameWithOutExt + ".bar")).getAbsolutePath();
